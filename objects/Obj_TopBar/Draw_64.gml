...
if (InFileBounds) {
	switch (room) {
		...
		case (Rm_FlrEditor) :
			...
			draw_text_transformed_color(50, 155, "Recent Files                                    >>", 1, 1, 0, c_red, c_red, c_red, c_red, 1); //global.SpriteLoadColor
			//Save
			if (global.spriteLoadedFloor && global.spriteLoadedDoor) {
				//                      Y-29      Y+5
				draw_rectangle_color(4, 176, 434, 210, FourtSelCol, FourtSelCol, FourtSelCol, FourtSelCol, false);
			}
			draw_text_transformed_color(50, 205, "Save                               (Ctrl + S)", 1, 1, 0, c_red, c_red, c_red, c_red, 1); //global.SpriteLoadColor
			//Close                 Y-29      Y+5
...
if (InEditBounds) {
	...
	switch (room) {
		...
		case (Rm_FlrEditor) :
			...
			draw_text_transformed_color(175, 85, "Reload Sheet                    (Ctrl + R)", 1, 1, 0, c_red, c_red, c_red, c_red, 1); //Print File
			draw_rectangle_color(131, 91, 559, 126, SeconSelCol, SeconSelCol, SeconSelCol, SeconSelCol, false);
			draw_text_transformed_color(175, 120, "Preview Floor                    (Ctrl + O)", 1, 1, 0, c_red, c_red, c_red, c_red, 1); //Print Fileint File
			break;
		case (Rm_PlrEditor) :
			draw_rectangle_color(131, 52, 559, 90, FirstSelCol, FirstSelCol, FirstSelCol, FirstSelCol, false);
			draw_text_transformed_color(175, 85, "Reload Sheet                    (Ctrl + R)", 1, 1, 0, c_red, c_red, c_red, c_red, 1);
			draw_rectangle_color(131, 91, 559, 126, SeconSelCol, SeconSelCol, SeconSelCol, SeconSelCol, false);
			draw_text_transformed_color(175, 120, "Preview Character          (Ctrl + P)", 1, 1, 0, c_red, c_red, c_red, c_red, 1);
			break;
	}
}
